...
if (typeof $raxQuery.$ === "undefined") {
    throw "RemoteGrid requires jquery module to be loaded";
}
if (!$raxQuery.$.fn.drag) {
    throw "RemoteGrid requires jquery.event.drag module to be loaded";
}
if (typeof Remote === "undefined") {
    throw "remote.core.js not loaded";
}
...
(function (jQuery) {
    ...
    (function ($) {
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////
        // RemoteGrid class implementation (available as Remote.Grid)
         ...
         **/
        function RemoteGrid(container, data, columns, options) {
            // settings
            var defaults = {
                ...
                autoHeight: false,
                editorLock: Remote.GlobalEditorLock,
                showHeaderRow: false,
                headerRowHeight: 25,
                showFooterRow: false,
                footerRowHeight: 25,
            ...
            var $container;
            var uid = "RemoteGrid_" + Math.round(1000000 * Math.random());
            var self = this;
            ...
            function init() {
                ...
                if ($container.length < 1) {
                    throw new Error("RemoteGrid requires a valid container, " + container + " does not exist in the DOM.");
                }
                ...
                treeColumns = new Remote.TreeColumns(columns);
                ...
                if (options.enableColumnReorder && !$.fn.sortable) {
                    throw new Error("RemoteGrid's 'enableColumnReorder = true' option requires jquery-ui.sortable module to be loaded");
                }
                ...
                $paneHeaderL = $("<div class='remote-pane remote-pane-header remote-pane-left' tabIndex='0' />").appendTo($container);
                $paneHeaderR = $("<div class='remote-pane remote-pane-header remote-pane-right' tabIndex='0' />").appendTo($container);
                $paneTopL = $("<div class='remote-pane remote-pane-top remote-pane-left' tabIndex='0' />").appendTo($container);
                $paneTopR = $("<div class='remote-pane remote-pane-top remote-pane-right' tabIndex='0' />").appendTo($container);
                $paneBottomL = $("<div class='remote-pane remote-pane-bottom remote-pane-left' tabIndex='0' />").appendTo($container);
                $paneBottomR = $("<div class='remote-pane remote-pane-bottom remote-pane-right' tabIndex='0' />").appendTo($container);

                // Append the header scroller containers
                $headerScrollerL = $("<div class='ui-state-default remote-header remote-header-left' />").appendTo($paneHeaderL);
                $headerScrollerR = $("<div class='ui-state-default remote-header remote-header-right' />").appendTo($paneHeaderR);
                ...
                if (treeColumns.hasDepth()) {
                    ...
                    for (var index = 0; index < treeColumns.getDepth() - 1; index++) {
                        $groupHeadersL[index] = $("<div class='remote-group-header-columns remote-group-header-columns-left' style='left:-1000px' />").appendTo($headerScrollerL);
                        $groupHeadersR[index] = $("<div class='remote-group-header-columns remote-group-header-columns-right' style='left:-1000px' />").appendTo($headerScrollerR);
                    }
                ...
                $headerL = $("<div class='remote-header-columns remote-header-columns-left' style='left:-1000px' />").appendTo($headerScrollerL);
                $headerR = $("<div class='remote-header-columns remote-header-columns-right' style='left:-1000px' />").appendTo($headerScrollerR);

                // Cache the header columns
                $headers = $().add($headerL).add($headerR);

                $headerRowScrollerL = $("<div class='ui-state-default remote-headerrow' />").appendTo($paneTopL);
                $headerRowScrollerR = $("<div class='ui-state-default remote-headerrow' />").appendTo($paneTopR);
                ...
                $headerRowL = $("<div class='remote-headerrow-columns remote-headerrow-columns-left' />").appendTo($headerRowScrollerL);
                $headerRowR = $("<div class='remote-headerrow-columns remote-headerrow-columns-right' />").appendTo($headerRowScrollerR);

                $headerRow = $().add($headerRowL).add($headerRowR);

                // Append the top panel scroller
                $topPanelScrollerL = $("<div class='ui-state-default remote-top-panel-scroller' />").appendTo($paneTopL);
                $topPanelScrollerR = $("<div class='ui-state-default remote-top-panel-scroller' />").appendTo($paneTopR);

                $topPanelScroller = $().add($topPanelScrollerL).add($topPanelScrollerR);

                // Append the top panel
                $topPanelL = $("<div class='remote-top-panel' style='width:10000px' />").appendTo($topPanelScrollerL);
                $topPanelR = $("<div class='remote-top-panel' style='width:10000px' />").appendTo($topPanelScrollerR);
                ...
                $viewportTopL = $("<div class='remote-viewport remote-viewport-top remote-viewport-left' tabIndex='0' hideFocus />").appendTo($paneTopL);
                $viewportTopR = $("<div class='remote-viewport remote-viewport-top remote-viewport-right' tabIndex='0' hideFocus />").appendTo($paneTopR);
                $viewportBottomL = $("<div class='remote-viewport remote-viewport-bottom remote-viewport-left' tabIndex='0' hideFocus />").appendTo($paneBottomL);
                $viewportBottomR = $("<div class='remote-viewport remote-viewport-bottom remote-viewport-right' tabIndex='0' hideFocus />").appendTo($paneBottomR);
                ...
                // footer Row
                $footerRowScrollerR = $("<div class='ui-state-default remote-footerrow' />").appendTo($paneTopR);
                $footerRowScrollerL = $("<div class='ui-state-default remote-footerrow' />").appendTo($paneTopL);
                ...
                $footerRowL = $("<div class='remote-footerrow-columns remote-footerrow-columns-left' />").appendTo($footerRowScrollerL);
                $footerRowR = $("<div class='remote-footerrow-columns remote-footerrow-columns-right' />").appendTo($footerRowScrollerR);
            ...
            function finishInitialization() {
                if (!initialized) {
                    ...
                    // for usability reasons, all text selection in RemoteGrid is disabled
                    ...
                    $container
                        .bind("resize.RemoteGrid", resizeCanvas);
                    $viewport
                    ...
                    $headerScroller
                        ...
                        .delegate(".remote-header-column", "mouseenter", handleHeaderMouseEnter)
                        .delegate(".remote-header-column", "mouseleave", handleHeaderMouseLeave);
                    $headerRowScroller
                    ...
                    $canvas
                        ...
                        .delegate(".remote-cell", "mouseenter", handleMouseEnter)
                        .delegate(".remote-cell", "mouseleave", handleMouseLeave);
            ...
            function setActiveViewportNode(element) {
                if (element) {
                    $activeViewportNode = $(element.target).closest('.remote-viewport');
                }
            }
            ...
            function createColumnFooter() {
                $footerRow.find(".remote-footerrow-column")
                  .each(function () {
                ...
                for (var i = 0; i < columns.length; i++) {
                    ...
                    var footerRowCell = $("<div class='ui-state-default remote-footerrow-column l" + i + " r" + i + "'></div>")
                      .data("column", m)
            ...
            function createColumnGroupHeaders() {
                ...
                for (var index = 0; index < $groupHeadersL.length; index++) {
                    ...
                    for (var indexGroup in groupColumns) {
                        ...
                        $("<div class='ui-state-default remote-group-header-column' />")
                          .html("<span class='remote-column-name'>" + m.name + "</span>")
                          .attr("id", "" + uid + m.id)
            ...
            function createColumnHeaders() {
                ...
                $headers.find(".remote-header-column")
                  .each(function () {
                      var columnDef = $(this).data("column");
                      if (columnDef) {
                          trigger(self.onBeforeHeaderCellDestroy, {
                ...
                $headerRow.find(".remote-headerrow-column")
                  .each(function () {
                      var columnDef = $(this).data("column");
                      if (columnDef) {
                ...
                for (var i = 0; i < columns.length; i++) {
                    ...
                    var header = $("<div class='ui-state-default remote-header-column' />")
                      .html("<span class='remote-column-name'>" + m.name + "</span>")
                    ...
                    if (m.sortable) {
                        header.addClass("remote-header-sortable");
                        header.append("<span class='remote-sort-indicator' />");
                    }
                    ...
                    if (options.showHeaderRow) {
                        var headerRowCell = $("<div class='ui-state-default remote-headerrow-column l" + i + " r" + i + "'></div>")
                            .data("column", m)
            ...
            function setupColumnSort() {
                $headers.click(function (e) {
                    ...
                    if ($(e.target).hasClass("remote-resizable-handle")) {
                        return;
                    }

                    var $col = $(e.target).closest(".remote-header-column");
                    if (!$col.length) {
                        return;
                    }
            ...
            function currentPositionInHeader(id) {
                ...
                $headers.find('.remote-header-column').each(function (i) {
                    if (this.id == id) {
                        currentPosition = i;
                        return false;
                    }
            ...
            function setupColumnReorder() {
                ...
                $headers.sortable({
                    ...
                    helper: "clone",
                    placeholder: "remote-sortable-placeholder ui-state-default remote-header-column",
                    start: function (e, ui) {
                        ...
                        $(ui.helper).addClass("remote-header-column-active");
                    },
                    beforeStop: function (e, ui) {
                        $(ui.helper).removeClass("remote-header-column-active");
                    },
                    sort: function (e, ui) {
            ...
            function setupColumnResize() {
                ...
                columnElements.find(".remote-resizable-handle").remove();
                ...
                columnElements.each(function (i, e) {
                    ...
                    $col = $(e);
                    $("<div class='remote-resizable-handle' />")
                      .appendTo(e)
                      .bind("dragstart", function (e, dd) {
                          ...
                          pageX = e.pageX;
                          $(this).parent().addClass("remote-header-column-active");
                      ...
                      .bind("dragend", function (e, dd) {
                          var newWidth;
                          $(this).parent().removeClass("remote-header-column-active");
            ...
            function measureCellPaddingAndBorder() {
                ...
                el = $("<div class='ui-state-default remote-header-column' style='visibility:hidden'>-</div>").appendTo($headers);
                ...
                var r = $("<div class='remote-row' />").appendTo($canvas);
                el = $("<div class='remote-cell' id='' style='visibility:hidden'>-</div>").appendTo(r);
            ...
            function createCssRules() {
                ...
                var rules = [
                    "." + uid + " .remote-group-header-column { left: 1000px; }",
                    "." + uid + " .remote-header-column { left: 1000px; }",
                    "." + uid + " .remote-top-panel { height:" + options.topPanelHeight + "px; }",
                    "." + uid + " .remote-headerrow-columns { height:" + options.headerRowHeight + "px; }",
                    "." + uid + " .remote-cell { height:" + rowHeight + "px; }",
                    "." + uid + " .remote-row { height:" + options.rowHeight + "px; }",
                    "." + uid + " .remote-footerrow-columns { height:" + options.footerRowHeight + "px; }"
                ];
            ...
            function setSortColumns(cols) {
                ...
                headerColumnEls
                    .removeClass("remote-header-column-sorted")
                    .find(".remote-sort-indicator")
                        .removeClass("remote-sort-indicator-asc remote-sort-indicator-desc");
                ...
                $.each(sortColumns, function (i, col) {
                    ...
                    if (columnIndex != null) {
                        headerColumnEls.eq(columnIndex)
                            .addClass("remote-header-column-sorted")
                            .find(".remote-sort-indicator")
                                .addClass(col.sortAsc ? "remote-sort-indicator-asc" : "remote-sort-indicator-desc");
                    }
                });
            }
            ...
            function setColumns(columnDefinitions) {
                var _treeColumns = new Remote.TreeColumns(columnDefinitions);
            ...
            function handleMouseWheel(e, delta, deltaX, deltaY) {
                var $rowNode = $(e.target).closest(".remote-row");
            ...
            function handleClick(e) {
                if (!currentEditor) {
                    ...
                    if (e.target != document.activeElement || $(e.target).hasClass("remote-cell")) {
                        setFocus();
                    }
                }
            ...
            function handleContextMenu(e) {
                var $cell = $(e.target).closest(".remote-cell", $canvas);
                if ($cell.length === 0) {
                    return;
                }
            ...
            function handleHeaderContextMenu(e) {
                var $header = $(e.target).closest(".remote-header-column", ".remote-header-columns");
            ...
            function rowsToRanges(rows) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ranges.push(new Remote.Range(rows[i], 0, rows[i], lastCell));
                }
                return ranges;
            }
            ...
            $.extend(this, {
                "RemoteGridVersion": "2.1",

                // Events
                "onScroll": new Remote.Event(),
                "onSort": new Remote.Event(),
                "onHeaderMouseEnter": new Remote.Event(),
                "onHeaderMouseLeave": new Remote.Event(),
                "onHeaderContextMenu": new Remote.Event(),
                "onHeaderClick": new Remote.Event(),
                "onHeaderCellRendered": new Remote.Event(),
                "onBeforeHeaderCellDestroy": new Remote.Event(),
                "onHeaderRowCellRendered": new Remote.Event(),
                "onFooterRowCellRendered": new Remote.Event(),
                "onBeforeHeaderRowCellDestroy": new Remote.Event(),
                "onBeforeFooterRowCellDestroy": new Remote.Event(),
                "onMouseEnter": new Remote.Event(),
                "onMouseLeave": new Remote.Event(),
                "onClick": new Remote.Event(),
                "onDblClick": new Remote.Event(),
                "onContextMenu": new Remote.Event(),
                "onKeyDown": new Remote.Event(),
                "onAddNewRow": new Remote.Event(),
                "onValidationError": new Remote.Event(),
                "onViewportChanged": new Remote.Event(),
                "onColumnsReordered": new Remote.Event(),
                "onColumnsResized": new Remote.Event(),
                "onCellChange": new Remote.Event(),
                "onBeforeEditCell": new Remote.Event(),
                "onBeforeCellEditorDestroy": new Remote.Event(),
                "onBeforeDestroy": new Remote.Event(),
                "onActiveCellChanged": new Remote.Event(),
                "onActiveCellPositionChanged": new Remote.Event(),
                "onDragInit": new Remote.Event(),
                "onDragStart": new Remote.Event(),
                "onDrag": new Remote.Event(),
                "onDragEnd": new Remote.Event(),
                "onSelectedRowsChanged": new Remote.Event(),
                "onCellCssStylesChanged": new Remote.Event(),
